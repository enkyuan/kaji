...
export const drizzleResolver = (options: DrizzleResolverOptions): Resolver => {
  ...
  return {
    ...
    handler: (ctx) => {
      ...
      const getType = getTypeFactory((field) => {
        const fieldName = convertToSnakeCase(field.fieldName, options.camelCase);
        ...
        return {
            ...
            provider === "sqlite"
              ...
              : field.unique
                ...
                : field.references
                  ...
                  : `t.text("${fieldName}")`,
          boolean:
            provider === "sqlite" ? `t.integer("${fieldName}")` : `t.boolean("${fieldName}")`,
          number,
          date:
            provider === "pg"
          ...
          foreignKeyId: ctx.useNumberId ? `t.integer("${fieldName}")` : `t.text("${fieldName}")`,
          "number[]": `${number}.array()`,
