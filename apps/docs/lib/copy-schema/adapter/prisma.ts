  ...
  return {
    ...
    handler: (ctx) => {
      const getType = getTypeFactory((field) => {
        ...
        let foreignKeyId = ctx.useNumberId && provider !== "mongodb" ? "Int" : "String";
      ...
      const lines = [`model ${capitalize(ctx.schema.modelName)}${options.usePlural ? "s" : ""} {`];
      if (ctx.mode === "alter") {
      ...
      lines.push(`\t@@map("${ctx.schema.modelName}${options.usePlural ? "s" : ""}")`);
      lines.push("}");
      return lines.join("\n");
    },
  };
