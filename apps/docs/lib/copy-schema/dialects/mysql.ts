import type { DBFieldAttribute, Resolver, ResolverHandlerContext } from "../types";
...
export const mysqlResolver = {
  handler: (ctx) => {
    const getType = getTypeFactory((field) => ({
      string: field.unique ? "varchar(255)" : field.references ? "varchar(36)" : "text",
      boolean: "boolean",
    ...
    for (const field of filterForeignKeys(ctx.schema)) {
      lines.push(`\t${ctx.mode === "alter" ? "ADD " : ""}${formatForeignKey(field)}`);
    }
