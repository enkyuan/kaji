...
export const copySchema = <const S extends DBSchema, O extends CopySchemaOptions>(
  schema: S,
  options: O,
) => {
  const conditions = new Set<string>();
  const resolver =
    typeof options.dialect !== "string" ? options.dialect : defaultResolvers[options.dialect];
  ...
  if (!mode) {
    mode = defaultModels.includes(filteredSchema.modelName) ? "alter" : "create";
  }
