...
import { ChevronDownIcon, History, PencilLine, Scale, Search } from "lucide-react";
...
const _CareersIcon: React.FC<{ className?: string }> = ({ className }) => (
  <svg
    xmlns="http://www.w3.org/2000/svg"
    width="1em"
    height="1em"
...
const _HorizontalLinesPattern: React.FC<{ className?: string }> = ({ className }) => {
  const rows = 40;
  const width = 100;
  const height = rows * 3;
  ...
  for (let i = 0; i < rows; i++) {
    const y = i * 3 + 1;
    lines.push(
      <line key={i} x1={0} y1={y} x2={width} y2={y} stroke="currentColor" strokeWidth="0.75" />,
    );
  }
  return (
    <svg
...
const VerticalLinesPattern: React.FC<{ className?: string }> = ({ className }) => {
  const cols = 72;
  const width = cols * 3;
  const height = 100;
  const lines: React.ReactElement[] = [];
...
const _CareersPattern: React.FC<{ className?: string }> = ({ className }) => {
  ...
  for (let r = 0; r < rows; r++) {
    for (let c = 0; c < cols; c++) {
      ...
      if (rand < density) {
        rects.push(
          <rect key={`${c}-${r}`} x={c * stride} y={r * stride} width={cell} height={cell} />,
        );
      }
    }
  }
                ...
                >
                  <div className="flex flex-col divide-y divide-foreground/[0.06]">
                    {products.map((p) => (
                      ...
                      >
                        {p.BgPattern && <p.BgPattern className={p.bgPatternClassName ?? ""} />}
                        {p.Pattern && (
                          <p.Pattern
                            className={
                              p.patternClassName ??
                  ...
                  <>
                    ...
                    <Accordion
                      ...
                      defaultValue={[
                        ...mobileMenuSections
                          .filter((s) =>
                            s.children?.some((item) => isActivePrefix(item.path || item.href)),
                          )
                          .map((s) => s.name),
                      ]}
                      className="w-full"
