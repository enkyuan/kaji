...
const footerLinks = [
  ...
  { label: "GitHub", href: "https://github.com/enkyuan/alloy" },
];
    ...
    <footer className="relative mt-10 py-6 px-5 sm:px-6 lg:px-8">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          ...
          {footerLinks.map((link, i) => (
            <span key={link.label} className="flex items-center">
              ...
              {i < footerLinks.length - 1 && (
                <span className="text-foreground/10 mx-1 text-[10px] select-none">/</span>
              )}
            </span>
          ))}
        </div>
        ...
        <div className="flex items-center justify-between w-full sm:w-auto sm:gap-4 shrink-0">
          <span className="text-[10px] text-foreground/50 font-mono">© 2026 agentkit</span>
